 ...
 * subclassed by a user
 * creating a robot program. Overridden autonomous() and operatorControl()
 * methods are called at the
 * appropriate time as the match proceeds. In the current implementation, the
 * Autonomous code will
 * run to completion before the OperatorControl code could start. In the future
 * the Autonomous code
...
public abstract class RobotBase implements AutoCloseable {
     ...
     * constructor that
     * runs before the Autonomous or Operator Control period starts. The constructor
     * will run to
     ...
     * future it would be
     * nice to put this code into its own task that loads on boot so ensure that it
     * runs.
     */
    protected RobotBase() {
    ...
    /**
     * Determine if the robot is current in Autonomous mode and enabled as
    ...
    /**
     * Determine if the robot is current in Operator Control mode and enabled as
     * determined by the
     * field controls.
     *
